...
| ADR-031 | **Trường plugin Better Auth định địa chỉ cột bằng property key của Drizzle, không phải tên cột database** | Plugin khai báo `fieldName: 'active_app_id'` — đúng tên cột trong database, và sai. Drizzle adapter tra cột bằng **property key** của đối tượng bảng (`activeAppId`); chính schema check của nó viết "each column by its property name". Adapter từ chối khởi động với `SCHEMA_MISMATCH`, `signInEmail` ném lỗi, endpoint bắt mọi lỗi từ nó và trả **401 "invalid email or password"** — nên một sự cố sập toàn phần đăng nhập email/mật khẩu **mang đúng bộ mặt của một mật khẩu gõ sai**, cho mọi người dùng. Drizzle đã sở hữu việc đặt tên cột; bỏ `fieldName` đi. Hàng rào: `packages/auth/tests/app-scope.plugin.test.ts`. | 2026-09-15 |
| ADR-032 | **Role mặc định theo app, mặc định `member`, `null` để tắt** | Thành viên app (`infra_app_members`) không phải là quyền: RBAC đọc quyền từ `infra_role_assignments`, và không có gì ghi vào đó. Người mới đăng nhập được, cầm token hợp lệ, và bị từ chối mọi thứ — đúng hành vi, nhưng từ bên ngoài không phân biệt được với sản phẩm hỏng. Một cột `default_role_key` phục vụ cả hai hình thái: B2C cần dùng được ngay, B2B cần mời-rồi-gán (`null`). Tự gán **fail open** ở mọi nhánh (app không thấy, key rỗng, role đã bị xoá) vì nó cấp tiện lợi chứ không cấp quyền truy cập — quyết định uỷ quyền vẫn fail closed ở `checkAccess`. | 2026-09-15 |
| ADR-033 | **Role mặc định không được mang quyền wildcard** | Lập luận "đã có ABAC phía sau nên tự gán an toàn" đúng một nửa. ABAC quyết định **chạm dòng nào**; RBAC quyết định **làm thao tác gì**. Một role mặc định mang `notes:delete` thì ổn — người ta chỉ xoá được dòng của chính mình. Một role mặc định mang `*` thì không: nó trao thao tác cho **mọi** tài khoản từng đăng ký, kể cả những tài khoản tạo ra sau khi người gõ nhầm đã quên chuyện đó. `setDefaultRole` từ chối `*`, `*:*`, `admin`, `admin:*`. | 2026-09-15 |